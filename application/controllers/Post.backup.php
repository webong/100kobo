			...
			if ($this->form_validation->run()) {
				...
				if ($this->input->post('reply_id') > 0) {
					...
					if ($image && !$this->m_posts->can_add_image()) {
						...
						// Pay certain price for commenting
						$post_access = $this->m_posts->get_post($post_id)[0]->trending;

						if ($post_access == 0) {
							$post_worth = $this->post_worth($post_id);
						} elseif ($post_access == 1) {
							$post_worth = $this->post_worth_trending($post_id);
						}

						$fixed_post_worth = $post_worth;
						...
						if ($wallet) {
							...
							// Add money to users
							$this->m_wallet->add_amount($user_id, 0, 50, 'wallet', 2); // Add to admin
							$post_worth = $post_worth - 50;
							$i = 0;
							$sub_posts = $this->m_posts->get_sub_posts($post_id);
							...
							foreach ($sub_posts as $post) {
								$this->m_wallet->add_amount($user_id, $post->user_id, 10, 'wallet', 2);
								$post_worth = $post_worth - 10;

								$user_email = $this->m_users->get_user_info($post->user_id)->email;

								$users_earns[$user_email] += 10;
							}
							
							foreach ($users_earns as $email => $value) {
								if ($email != $this->m_users->get_user_info($user_id)->email) {
									$this->email->to($email);
									$this->email->subject("You have earned $value kobo");
									$message = "<p>You have earned <strong>$value</strong> kobo from <a href=\"".base_url()."user/".$this->m_users->get_user_info($user_id)->username."\">".$this->m_users->get_user_info($user_id)->username."<a/></strong>.</p>";
							...
							if ($post_worth > 0) {
								$this->m_wallet->add_amount($user_id, 0, $post_worth, 'wallet', 2);
							}

							$this->session->set_flashdata('message_s','Posted successfully');
							
						} else {
							$this->session->set_flashdata('message_f','You don\'t have enough coins ('.$fixed_post_worth.')');
						}
					}
				}
				...
				foreach ($data_2['posts'] as $post) {
					...
					if ($temp_sub_post = $this->m_posts->get_sub_posts($post->id)) {
						foreach ($temp_sub_post as $sub_post) {
							...
							$sub_post->comment_worth = $this->post_worth($sub_post->id) / 100;
